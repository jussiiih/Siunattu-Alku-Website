    ...
    try {
        ...
        messages.forEach(message => {
            if (message.name) {
                ...
                const decipher = crypto.createDecipheriv('aes-256-cbc', config.CRYPT_KEY, Buffer.from(encryptedName.iv, 'hex'))
            ...
            if (message.phoneNumber) {
                ...
                const decipher = crypto.createDecipheriv('aes-256-cbc', config.CRYPT_KEY, Buffer.from(encryptedPhoneNumber.iv, 'hex'))
            ...
            if (message.email) {
                ...
                const decipher = crypto.createDecipheriv('aes-256-cbc', config.CRYPT_KEY, Buffer.from(encryptedEmail.iv, 'hex'))
            ...
            if (message.content) {
                ...
                const decipher = crypto.createDecipheriv('aes-256-cbc', config.CRYPT_KEY, Buffer.from(encryptedContent.iv, 'hex'))
...
messagesRouter.post('/', async (request, response) => {
    ...
    function encrypt(text) {
        ...
        const cipher = crypto.createCipheriv(algorithm, config.CRYPT_KEY, iv)
